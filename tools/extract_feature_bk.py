...
def main(args):
    ...
    im_base_name = os.path.basename(next(im_list))
    ...
    print(result['boxes'], result['region_feat'].shape)
        ...
        with h5py.File(os.path.join(args.box_output_dir, args.output_file_prefix + '_bbox' + proc_split + '.h5'), "w") as f, \
            ...
            for i, im_name in enumerate(im_list):
                ...
                if image_id[-4 - len(proc_split):-4] == proc_split:
                    ...
                    ic(result, proposals, result['region_feat'].shape, proposals.shape)
                    f.create_dataset(
                        image_id[:-4], data=proposals.astype(args.data_type))
                    f_feat.create_dataset(
                        image_id[:-4], data=result['region_feat'].squeeze().astype(args.data_type))
                    f_cls.create_dataset(
                        image_id[:-4], data=result['cls_prob'].astype(args.data_type))

                    p = '/mlcv/Databases/Imagecaption/Dataset/TrainValidTest/VLP_format/region_feat_gvd_wo_bgd/images'
